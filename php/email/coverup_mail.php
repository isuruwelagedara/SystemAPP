...
//$applied_leaves_id
///////// finding the cover up epf no from applyleave
    ...
    foreach ($results as $row) {
        ...
        $coverup_epf = $row['coverup_by'];
		$reason = $row['reason'];
		$requested_user = $row['name_ini'];
		
        
    }


///////// finding the coverup's name
    $sql = "SELECT * FROM users WHERE epf_no=$coverup_epf ";
    $query = $DBcon->prepare($sql);
    $query->execute();
    $results = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($results as $row) {
        ...
        $coverup_name = $row['name_ini'];
		$coverup_email = $row['email'];
...
$mail->addAddress($coverup_email, $coverup_name);
...
$mail->Subject = 'Headstart Leave System - Coverup';
                            ...
                            <tr>
                                <td style="word-wrap:break-word;font-size:0px;padding:0px 18px 0px 18px;" align="center">
                                    <div style="cursor:auto;color:#FF0000;font-family:Roboto, Tahoma, sans-serif;font-size:11px;line-height:22px;text-align:center;">
                                        <p>
                                            <span style="font-size:48px;">Hello,'.$coverup_name.'</span>
                                        </p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td style="word-wrap:break-word;font-size:0px;padding:0px 20px 0px 20px;" align="center">
                                    <div style="cursor:auto;color:#5F5F5F;font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:11px;line-height:22px;text-align:center;">
                                        <p>
                                            <span style="font-size:22px;">'.$requested_user.' is requested a leave </span>
                                        </p>
                                    </div>
                                </td>
                            </tr>
